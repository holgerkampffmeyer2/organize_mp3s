...
## Overview
This script organizes MP3 and M4A files by moving them to genre-specific folders based on online lookup of artist and title (extracted from file metadata).
If artist or title metadata is missing, the file is left in place and logged.
If online lookup fails to find a genre, the file is left in place and logged.
If the genre is not mapped in config.json, the file is left in place and logged.
If the target file already exists, the file is left in place and logged.

## Configuration
- `config.json`: JSON file mapping genres to target directories.
  - Keys can be comma-separated lists of genres (e.g., "Techno, House").
  - Example:
    ```json
    {
      "genre_map": {
...
2. **Extract Metadata**:
   ...
   - If either tag is missing or unreadable → leave file, log as `missing_metadata_<field>`.
3. **Online Lookup**:
   - Query iTunes Search API: `https://itunes.apple.com/search?term=<artist>+<title>&limit=1`
   ...
   - If both sources fail → leave file, log as `lookup_failed`.
4. **Genre Normalization & Mapping**:
   - Normalize genre to lowercase.
   - Special case: if genre contains both "drum" and "bass" → map to "Drum n Base" (if present in config).
   - Otherwise, look for exact match in `config.json` genre_map (keys are normalized to lowercase).
   - If no mapping found → leave file, log as `genre_not_mapped`.
5. **Destination Check**:
...
7. **Logging**:
   ...
   - Log entries include: file path, artist, title, detected genre (if available), reason (or action/destination in dry-run).
